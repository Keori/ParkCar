...
Calendar.prototype.draw = function() {
	...
	this.form.clearButton.onclick = function() {
		...
		calendar.getElementById(calendar.dateControl.attributes["id"].nodeValue
				+ "_show").innerHTML = calendar.dateControl.value;
		calendar.hide();
	}
		...
		if (dateArray[i]) {
			tds[i].onclick = function() {
				if (calendar.dateControl) {
					...
					calendar
							.getElementById(calendar.dateControl.attributes["id"].nodeValue
									+ "_show").innerHTML = calendar.dateControl.value;
				}
				calendar.hide();
			}
...
Date.prototype.format = function(style) {
	...
	if (/(y+)/.test(style)) {
		style = style.replace(RegExp.$1, (this.getFullYear() + "").substr(4
						- RegExp.$1.length));
	}
	for (var k in o) {
...
__ci.document.writeln('body {font-family:Helvetica,Arial,Tahoma;font-size:12px;margin:0px;text-align:center;}');
...
__ci.document.writeln('td.sat {color:#18A3A3;background-color:#EFEFEF;}');
__ci.document.writeln('td.sun {color:#FF0000;background-color:#EFEFEF;}');
__ci.document.writeln('td.normal {background-color:#EFEFEF;}');
__ci.document
		.writeln('input.l {line-height:16px;border:1px solid #CCCCCC;background-color:#EFEFEF;width:20px;height:20px;}');
__ci.document
		.writeln('input.r {line-height:16px;border:1px solid #CCCCCC;background-color:#EFEFEF;width:20px;height:20px;}');
__ci.document
		.writeln('input.b {line-height:16px;border:1px solid #CCCCCC;background-color:#EFEFEF;width:100%;height:20px;}');
__ci.document.writeln('-->');
